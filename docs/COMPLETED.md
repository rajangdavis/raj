...
## Attach mirrors the daemon; a decision wakes the watch (2026-09-20)

- [x] **Attach mirrors the daemon, and a local close snoozes (2026-09-20).** `StartClient` mirrors every daemon real tab (headless buffers stay hidden) and `syncClient` keeps mirroring continuously, so a tab the daemon opens after the attach appears and one it closes is dropped; `adoptPending` is unconditional (no review-tabs gate), so a host buffer with a pending change set becomes a client tab even when the client never opened it. A local close records a `bufferMark` snooze, persisted per client key in the saved view with a legacy list-of-paths fallback, and re-opens once the buffer's version or review facts change; `--review-tabs` and `Options.ReviewTabs`/`ReviewTabsSet` are removed.
- [x] **A decision-only change wakes the watch (2026-09-20).** `App.reviewGeneration` replaces `bufferVersionHash` and folds each tab/headless buffer's path, session version, `DecisionGeneration()` and `ViewDirty()`, plus the sorted pending-deletion and dir-removal memberships, so `controlTick` wakes watchers on an accept/reject/clear, a bare save or a workspace removal even though none moves `Session.Version`.

## Control surface simplification; daemon restart and status (2026-09-20)

- [x] **Control is always on, with one address spelling (2026-09-20).** `--control`, `--control-socket` and `--control-exec` are removed; an editor and a daemon both serve the `DefaultPath()` Unix socket (overridable with `RAJ_CONTROL_SOCKET`) plus a loopback `tcp://127.0.0.1:7391`, and `--control-addr` widens that port. A taken port fails the bind rather than bumping, so the address a client was told is the address it reaches. `exec` is refused over TCP. `--attach` dials `--control-addr` only when the user passed it, otherwise discovery finds the socket.
- [x] **`raj daemon restart` (2026-09-20).** Stops and starts the workspace daemon, carrying the recorded TCP address and token across (`RAJ_CONTROL_TOKEN` set once in the child environment) unless `--control-addr` overrides the address; the per-process socket is always the new default.
- [x] **`raj ctl status` (2026-09-20).** A CLI-only verb over the `buffers` op: exit 0 when every buffer is saved and no set is pending, exit 1 naming every dirty or pending buffer with its counts; `-json` carries the same `ready`/`blocking` facts and exit code. Deliberately not wired into `make check`.
- [x] **A control-driven change wakes watchers in the same beat (2026-09-20).** `drainControl` calls `controlTick` after a batch, so a decision or edit landed over the socket wakes parked client watches without waiting for the next idle tick; the idle tick stays the backstop for changes made outside a control request.
- [x] **A client tab shows review work, not snapshot dirt (2026-09-20).** A snapshot/client tab marks `•` when `Session().Pending() > 0` rather than on `ViewDirty` (true for most non-empty snapshots), so a clean client tab has no dot and the local editor's save-oriented dot is unchanged.

## Flag spelling and daemon flag forwarding (2026-09-20)
