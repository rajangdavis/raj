...
4. **Lease and error recovery.** Writing over an existing proposal refuses with "change set N owns this text; accept or reject it first"; recovery is reject → `clear` → re-apply. `clear` exists but is documented only on its usage line. The session's reported `accepted 0 ops` artifact did not reproduce (the string is absent from the tree).
5. **Output shape.** `search -json`, `stats -json` and `run -json` marshal Go field names (`Path`, `ByteStart`, `LineStart` / `Runs`, `Stale`, `AgentOnly`) while `read`/`buffers`/`groups`/`diff`/`version`/`whoami`/`apply`/`lsp` use snake_case; `truncated` mixes `Path`/`Shown`/`Total` under lowercase keys. The contract is unstated.
